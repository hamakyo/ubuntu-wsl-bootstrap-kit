...
採用ポートフォリオの観点では、「運用目線で開発基盤を設計し、継続運用を考慮している」ことを示す構成です。
...
1. Ubuntu/WSL onboarding template with Dev Container, cloud-init, and
   Ansible for reproducible developer environments.
2. Platform/DevEx portfolio template: bootstrap local Ubuntu/WSL,
   provision VM first-boot, and apply continuous config as code.
